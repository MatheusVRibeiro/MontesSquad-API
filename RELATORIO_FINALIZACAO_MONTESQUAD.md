...
- **B12 (🟠 ALTO — CORRIGIDO, commits `46a7f2e` backend + `dddb479` frontend):** `perfil.tsx` inicializava `name` do localStorage — se o user persistido estivesse com `nome: ""`, o form abria vazio e o `save()` enviava `{nome:""}` → PATCH zerava o nome no banco. Fix (Opção B escolhida pelo usuário): frontend monta o formulário com `fetchMe()` (nova rota `GET /usuarios/me`) em vez de depender do localStorage; backend ganhou coluna `avatar_url` (migration `scripts/migrar_avatar_url.js` + 3 SQLs sincronizados), PATCH aceita `avatar_url`, login retorna bio/localizacao/avatar_url. **Bonus GitHub:** "Importar do GitHub" agora traz também a foto (`avatar_url`), além de nome/bio/localização/skills; Avatar renderiza imagem com fallback de iniciais. Validado no navegador: salvar perfil com bio nova preservou nome/localização/avatar. 32/32 + 29/29 testes verdes.
