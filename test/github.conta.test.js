...
// Stub do githubApp (mesmo padrão dos testes anteriores) — evita dependência de env
...
describe("ETAPA 2 — Conectar/desconectar GitHub (regra de senha)", () => {
  ...
  it("GET /github/connect — conta local inicia conexão (200 + URL OAuth)", async () => {
    ...
    // state carrega o usuário logado (vínculo pós-login)
    const decoded = jwt.verify(res.body.dados.state, process.env.JWT_SECRET);
    expect(decoded.oauth).toBe("github");
    expect(decoded.uid).toBe(5);
  });

  it("GET /github/callback-link — alias de connect (200 + URL OAuth pós-login)", async () => {
    const res = await request(app).get("/github/callback-link").set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(200);
    expect(res.body.sucesso).toBe(true);
    expect(res.body.dados.url).toContain("github.com/login/oauth/authorize");
    const decoded = jwt.verify(res.body.dados.state, process.env.JWT_SECRET);
  ...
  it("GET /github/me — expõe senha_definida e cadastro_origem para o frontend", async () => {
    const res = await request(app).get("/github/me").set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(200);
    expect(res.body.dados.conectado).toBe(true);
    expect(res.body.dados.senha_definida).toBe(true);
    expect(res.body.dados.cadastro_origem).toBe("local");
  });
});
