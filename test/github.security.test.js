...
function criarPool() {
  return criarPoolFake([
    ...
    { match: (sql) => /^select github_user_id, github_login, github_avatar_url, github_connected_at,\s+senha_definida, cadastro_origem from usuarios where id = \? limit 1$/.test(sql), resposta: () => [[{ github_user_id: null, github_login: null, github_avatar_url: null, github_connected_at: null, senha_definida: 1, cadastro_origem: "local" }], []] },
